...
export function useMedicalRecord(patientId: string): UseMedicalRecordResult {
  const [dossierMedicalDTO, setDossierMedicalDTO] =
    useState<DossierMedicalDTO | null>(null);
  const [medicalRecord, setMedicalRecord] = useState<DossierMedical | null>(
    null
  );
  ...
  const fetchMedicalRecord =
    useCallback(async (): Promise<DossierMedical | null> => {
      if (!patientId) {
      ...
      try {
        const dossier =
          await dossierMedicalService.getDossierMedicalByPatientId(patientId);
  ...
  const createMedicalRecord = async (
    data: Partial<DossierMedicalDTO>
  ): Promise<void> => {
    if (!patientId) {
    ...
    try {
      ...
      const newMedicalRecord = await dossierMedicalService.createDossierMedical(
        dossierMedicalDTO
      );
