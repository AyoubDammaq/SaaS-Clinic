...
export const medicalRecordService = {
  ...
  updateMedicalRecord: async (id: string, data: Partial<Omit<MedicalRecord, "id" | "patientId">>): Promise<MedicalRecord> => {
    try {
      // In production, this would use the real API
      ...
      return {
        ...
        currentMedications: data.currentMedications || 'Ventolin (2 puffs daily), Lisinopril 10mg',
        familyHistory: data.familyHistory || 'Father: Diabetes, Mother: Breast cancer',
        personalHistory: data.personalHistory || 'Appendectomy (2018), Fractured arm (2015)',
  ...
  addDocument: async (medicalRecordId: string, document: Omit<Document, "id">): Promise<Document> => {
    try {
      // In production, this would use the real API
