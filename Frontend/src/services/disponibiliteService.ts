...
export const disponibiliteService = {
  ...
  async getAllAvailabilities(): Promise<Disponibilite[]> {
    const response = await api.get<Disponibilite[]>(
      API_ENDPOINTS.Disponibilite.GET_ALL
    );
    return response;
  },
  ...
  async getAvailabilitiesByDoctor(doctorId: UUID): Promise<Disponibilite[]> {
    const response = await api.get<Disponibilite[]>(
      API_ENDPOINTS.Disponibilite.GET_BY_DOCTOR(doctorId)
    );
    return response;
  },
  ...
  async addDisponibilite(
    Disponibilite: Omit<Disponibilite, "id">
  ): Promise<Disponibilite> {
    const response = await api.post<Disponibilite>(
      API_ENDPOINTS.Disponibilite.ADD(),
      Disponibilite
    );
    return response;
  },
  ...
  async updateDisponibilite(
    ...
    updatedData: Partial<Disponibilite>
  ): Promise<Disponibilite> {
    const response = await api.put<Disponibilite>(
      API_ENDPOINTS.Disponibilite.UPDATE(DisponibiliteId),
      updatedData
    );
    return response;
  },
  ...
  async deleteDisponibilite(DisponibiliteId: UUID): Promise<void> {
    try {
      await api.delete<void>(
        API_ENDPOINTS.Disponibilite.DELETE(DisponibiliteId)
      );
    } catch (error) {
  ...
  async deleteAvailabilitiesByDoctor(doctorId: UUID): Promise<void> {
    try {
      await api.delete<void>(
        API_ENDPOINTS.Disponibilite.DELETE_BY_DOCTOR(doctorId)
      );
    } catch (error) {
  ...
  async getDisponibiliteByDay(
    doctorId: UUID,
    day: string
  ): Promise<Disponibilite[]> {
    const response = await api.get<Disponibilite[]>(
      API_ENDPOINTS.Disponibilite.GET_BY_DAY(doctorId, day)
    );
    return response;
  },
  ...
  async getAvailableSlots(
    ...
    const response = await api.get<CreneauDisponibleDto[]>(
      API_ENDPOINTS.Disponibilite.GET_AVAILABLE_SLOTS(doctorId, date)
    );
    return response;
  },
  ...
  async isDoctorAvailable(doctorId: UUID, dateTime: string): Promise<boolean> {
    const response = await api.get<boolean>(
      API_ENDPOINTS.Disponibilite.IS_AVAILABLE(doctorId, dateTime)
    );
    return response;
  },

  // Get total available time for a doctor between two dates
  async getTotalAvailableTime(
    doctorId: UUID,
    start: string,
    end: string
  ): Promise<number> {
    const response = await api.get<number>(
      API_ENDPOINTS.Disponibilite.TOTAL_AVAILABLE_TIME(doctorId, start, end)
    );
    return response;
  },

  // Get availabilities for a doctor in a specific interval
  async getAvailabilitiesInInterval(
    doctorId: UUID,
    start: string,
    end: string
  ): Promise<Disponibilite[]> {
    const response = await api.get<Disponibilite[]>(
      API_ENDPOINTS.Disponibilite.GET_IN_INTERVAL(doctorId, start, end)
    );
    return response;
  },
};
