 ...
 */
export async function apiClient<T = unknown>(url: string, options: RequestOptions): Promise<T> {
  try {
    const { 
      method, 
      body, 
    ...
    console.log(`API Request to ${url}`, { 
      ...
      headers: { ...headers, Authorization: withAuth ? 'Bearer ***' : undefined } 
    });
    ...
    if (response.status === 429) {
      ...
      if (response.headers.get('Retry-After')) {
        ...
        const exponentialDelay = Math.min(30000, retryDelay * Math.pow(2, retryCount)); // Max 30 seconds
        waitTime = exponentialDelay * (0.8 + Math.random() * 0.4); // Add jitter (±20%)
      }
      
      if (retryCount < maxRetries) {
        console.warn(`Rate limited. Retrying after ${waitTime/1000} seconds. Retry attempt ${retryCount + 1}/${maxRetries}`);
        toast.warning(`API rate limit reached. Retrying in ${Math.ceil(waitTime/1000)} seconds...`);
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorMessage = errorData.message || errorData.title || `Error: ${response.status} ${response.statusText}`;
      } catch (parseError) {
...
export const api = {
  ...
  post: <T = unknown>(url: string, body?: unknown, options?: Partial<RequestOptions>) =>
    apiClient<T>(url, { method: HTTP_METHODS.POST, body, ...options }),

  put: <T = unknown>(url: string, body?: unknown, options?: Partial<RequestOptions>) =>
