...
import { Bell, Calendar, FileText, MessageCircle, Settings, CheckCircle, Eye } from 'lucide-react';
...
const mapNotificationDtoToNotification = (dto: NotificationDto): Notification => {
  ...
  const priorityMap: Record<BackendNotificationPriority, Notification['priority']> = {
...
export function NotificationDropdown({ onClose }: NotificationDropdownProps) {
  ...
  const { user } = useAuth();
  const { filteredNotifications, isLoading, isSubmitting, permissions, markNotificationAsRead, markAllNotificationsAsRead, setSelectedNotification } = useNotifications();
  const navigate = useNavigate();
  const [frontendNotifications, setFrontendNotifications] = useState<Notification[]>([]);
  const [selectedFrontendNotification, setSelectedFrontendNotification] = useState<Notification | null>(null);

  // Map backend notifications to frontend format
  useEffect(() => {
    const mappedNotifications = filteredNotifications.map(mapNotificationDtoToNotification);
  ...
  const handleNotificationClick = (notification: Notification) => {
    ...
    const backendNotification = filteredNotifications.find(n => n.id === notification.id) || null;
      ...
      <div className="p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {unreadCount > 0
              ? t('unread_notifications_other', { values: { count: unreadCount } }).replace('{count}', unreadCount.toString())
              : t('all_up_to_date')}
          </p>
        </div>
