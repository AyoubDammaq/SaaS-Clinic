...
export const AppointmentFilters = ({
  ...
  const hasActiveFilters = dateFilter || statusFilter !== "all" || clinicFilter !== "all" || doctorFilter !== "all" || patientFilter !== "all";
  ...
  const showDoctorFilter = userRole === 'Patient' || userRole === 'ClinicAdmin' || userRole === 'SuperAdmin';
  const showPatientFilter = userRole === 'Doctor' || userRole === 'ClinicAdmin' || userRole === 'SuperAdmin';
  const showStatusFilter = userRole === 'Patient' || userRole === 'Doctor' || userRole === 'ClinicAdmin' || userRole === 'SuperAdmin';
  const showDateFilter = userRole === 'Patient' || userRole === 'ClinicAdmin' || userRole === 'SuperAdmin' || userRole === 'Doctor';
      ...
      {hasActiveFilters && !filtersExpanded && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {patientFilter !== "all" && showPatientFilter && (
            <Badge variant="secondary" className="flex gap-1 items-center">
              {patients.find(p => p.id === patientFilter)?.nom || patientFilter}
              <X 
                className="h-3 w-3 ml-1 cursor-pointer" 
