...
import { resources, Resource, TranslationValues, TranslationKey } from './locales';
import { Language } from '@/contexts/LanguageContext';

// Re-export TranslationValues type
export type { TranslationValues, TranslationKey };

export function getTranslation<R extends Resource = 'common'>(key: string, language: string): string {
  ...
  for (const namespace of Object.keys(resources.en) as Resource[]) {
    if (resources[lang] && 
      ...
      return (resources[lang][namespace] as Record<string, string>)[key as string];
    }
  }
  
  // Fall back to English
  for (const namespace of Object.keys(resources.en) as Resource[]) {
    if (resources.en[namespace] && (resources.en[namespace] as Record<string, string>)[key as string]) {
